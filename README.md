...
Mattermost client for [opencode](https://opencode.ai). It gives the agent eleven Mattermost
tools: read and post messages, browse channels, search, download attachments.
...
Do not skip the `permission` block. The four write tools raise a permission request, but with no
...
The last four are the write tools gated by the `permission` block above. `mattermost_mark_read`
also changes server state but is deliberately ungated — it only clears your own unread markers.
...
working tree as a plugin instead of the published package, and marks the four write tools as `ask`:
